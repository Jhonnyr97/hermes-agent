...
Important safety rule: cron-run sessions should not recursively schedule more cron jobs.""",
    "parameters": {
        ...
        "properties": {
            ...
            "deliver": {
                "type": "string",
                "description": "Omit this parameter to auto-deliver back to the current chat and topic (recommended). Auto-detection preserves thread/topic context. Only set explicitly when the user asks to deliver somewhere OTHER than the current conversation. Values: 'origin' (same as omitting), 'local' (no delivery, save only), 'all' (fan out to every connected home channel), 'web' (deliver to the AziendaOS web UI via the cron delivery endpoint), or platform:chat_id:thread_id for a specific destination. Combine with comma: 'origin,all' delivers to the origin plus every other connected channel. Examples: 'telegram:-1001234567890:17585', 'discord:#engineering', 'sms:+15551234567', 'all', 'web'. WARNING: 'platform:chat_id' without :thread_id loses topic targeting. 'all' resolves at fire time, so a job created before a channel was wired up will pick it up automatically once connected.\n\nWhen 'web' is used, origin is automatically set from the environment (HERMES_WEB_ORIGIN_PLATFORM, HERMES_WEB_CHAT_ID). The web UI's delivery endpoint receives the output. If the target session was deleted, the UI auto-creates a new session and shows a badge indicating the output came from a cron job."
            },
            "skills": {
                "type": "array",
                "items": {"type": "string"},
