...
class APIServerAdapter(BasePlatformAdapter):
    ...
    async def _handle_runs(self, request: "web.Request") -> "web.Response":
        ...
        async def _run_and_close():
            try:
                ...
                def _clarify_cb(question: str, choices: Optional[List[str]] = None) -> str:
                    import uuid as _uuid
                    from tools.clarify_gateway import register, wait_for_response, get_clarify_timeout

                    clarify_id = _uuid.uuid4().hex
                    register(clarify_id, approval_session_key, question, choices)
                    self._run_clarify_session_keys[run_id] = approval_session_key
                    self._run_clarify_ids[(run_id, clarify_id)] = approval_session_key
                    self._clarify_session_map[clarify_id] = approval_session_key
                    ...
                    try:
                        ...
                        pass
                    response = wait_for_response(clarify_id, timeout)
                    return response or ""

                def _clarify_notify(entry) -> None:
                    self._run_clarify_session_keys[run_id] = approval_session_key
                    self._run_clarify_ids[(run_id, entry.clarify_id)] = approval_session_key
                    self._clarify_session_map[entry.clarify_id] = approval_session_key
                    timeout = 300
                    try:
                ...
                def _approval_notify(approval_data: Dict[str, Any]) -> None:
                    ...
                    self._run_approval_session_keys[run_id] = approval_session_key
                    self._run_approval_ids[(run_id, approval_id)] = approval_session_key
                    self._approval_session_map[approval_id] = approval_session_key
                ...
                def _run_sync():
                    ...
                    from tools.clarify_gateway import register_notify, unregister_notify
                    ...
                    try:
                        ...
                        register_notify(approval_session_key, _clarify_notify)
                        ...
                        try:
                            ...
                            unregister_notify(approval_session_key)
                        finally:
                ...
                for key in [k for k in list(self._run_approval_ids) if k[0] == run_id]:
                ...
                for key in [k for k in list(self._run_clarify_ids) if k[0] == run_id]:
                    self._run_clarify_ids.pop(key, None)
                    self._clarify_session_map.pop(key[1], None)
    ...
    async def _handle_run_approval(self, request: "web.Request") -> "web.Response":
        ...
        session_key = self._run_approval_session_keys.get(run_id) or self._run_approval_sessions.get(run_id)
        ...
        if approval_id and self._run_approval_ids.get((run_id, approval_id)) != session_key:
            return web.json_response(
        ...
        logger.info(
            ...
            run_id, approval_id or "(fifo)", decision,
        )
        return web.json_response({
            "run_id": run_id,
            "status": "processed",
    ...
    async def _handle_clarify_response(self, request: "web.Request") -> "web.Response":
        ...
        from tools.clarify_gateway import has_pending, resolve_gateway_clarify

        if not has_pending(session_key):
            return web.json_response(
    ...
    async def _handle_file_read(self, request: "web.Request") -> "web.Response":
        ...
        # Size limit. Web clients use this endpoint to retrieve attach_file artifacts;
