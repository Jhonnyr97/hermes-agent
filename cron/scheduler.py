...
def _deliver_result(job: dict, content: str, adapters=None, loop=None) -> Optional[str]:
    ...
    for target in targets:
        ...
        if not platform:
            # Handle aziendaos delivery: POST directly to Rails UI endpoint
            if platform_name.lower() == "aziendaos":
                rails_url = os.getenv("AZIENDAOS_UI_URL", "http://hermes-ui:4000")
                try:
                    ...
                    logger.info("Job '%s': delivered to aziendaos:%s", job["id"], chat_id)
                    delivered = True
                except Exception as e:
                    msg = f"aziendaos delivery failed: {e}"
                    logger.warning("Job '%s': %s", job["id"], msg)
